...
#define GF(t,n) n = (t)SDL_GL_GetProcAddress(#n[sizeof(#n)-2]=='_'?"glActiveTexture":#n);
  GLFUNCS
#undef GF
#define BF(n) p_##n = (typeof(p_##n))SDL_GL_GetProcAddress(#n);
  GLBASEFUNCS
#undef BF
}
...
static void gen_textures(int bm){
  ...
  if(bm==2){
    ...
    for(int y=0;y<TS;y++)for(int x=0;x<TS;x++){
      float u=x/(float)TS, v=y/(float)TS;
      int gx=x&63, gy=y&31;
      float slot=(gx<5||gx>58)?0.0f:1.0f;
      float seam=(gy<2||gy>29)?0.0f:1.0f;
      float rib=fabsf(sinf((u*6.0f+fbm(u,v,3,4,1801u)*0.8f)*PI));
      float oil=fbm(u*1.2f,v*1.2f,5,4,1807u);
      float h=0.18f+slot*seam*0.42f+rib*0.22f+oil*0.18f;
      if(!slot||!seam)h*=0.30f;
      hh[y*TS+x]=h;
      float base=0.035f+0.075f*oil+0.025f*rib;
      if(!slot||!seam)base*=0.42f;
      putrgb(&alb[(y*TS+x)*4],base*0.55f,base*0.90f,base*0.82f+0.025f*oil);
    }
    h2n(hh,nrm,3.5f);
    ...
    for(int y=0;y<TS;y++)for(int x=0;x<TS;x++){
      ...
      float arch=fabsf(sinf((v*4.0f+fbm(u*1.5f,v,3,4,1901u))*PI));
      float pipe=((py>18&&py<27)||(py>42&&py<50))?0.9f:0.05f;
      float riv=0;
      for(int cx=16;cx<128;cx+=32){ float rx=px_-cx, ry=py-8; riv+=expf(-(rx*rx+ry*ry)/9.0f); }
      float oil=fbm(u+4.0f,v+1.0f,5,4,1907u);
      float h=0.20f+pipe*0.55f+arch*0.25f+0.40f*riv+0.18f*oil;
      hh[y*TS+x]=h;
      float base=0.045f+0.05f*oil+0.035f*pipe;
      putrgb(&alb[(y*TS+x)*4],base*0.62f,base*0.86f+0.02f*pipe,base*0.95f+0.02f*arch);
    }
    h2n(hh,nrm,3.4f);
...
static const unsigned char font[40][7]={
 ...
 {0x18,0x19,0x02,0x04,0x08,0x13,0x03},{0x00,0x04,0x04,0x1F,0x04,0x04,0x00}};

static float textw(const char*s,float sc){ int n=0;volatile const char*p=s;while(*p++){n++;}return n*6*sc; }
static void draw_text(float x,float y,float sc,const char*s){
  glBegin(GL_QUADS);
  for(;*s;s++,x+=6*sc){
    int gi=-1; char c=*s;
    if(c>='0'&&c<='9')gi=c-'0'; else if(c>='A'&&c<='Z')gi=10+c-'A'; else if(c=='-')gi=36; else if(c=='_')gi=37; else if(c=='%')gi=38; else if(c=='+')gi=39;
    if(gi<0)continue;
...
static int curse;                              /* 0 clean 1 blackout 2 swarm 3 famine 4 hunt */
typedef struct { int x,y,w,h; } Room;
static Room rooms[12]; static int nrooms;
static float startx,startz, exitx,exitz;
/* floor 8+ optional stalker: unkillable, random visits, freezes when shot */
static int stOn,stMode; /* 0 gone/wait, 1 warning, 2 hunting */
static float stx,stz,styaw,stT,stFreeze,stHitCD,stHop,stAnim;
static int bossFloor,pperk;
...
typedef struct { float x,z; int type; int taken; } Item;  /* 0 hp 1 bullets 2 shells 3 rockets 4 speed 5 rage */
...
static const EKind ekind[EK_COUNT]={
  ...
  /*BRUISER */{ 58, 1.45f,1.90f, 11.f,0.76f, 0.12f,0.10f,0.08f},
  /*A-SPIT  */{ 36, 2.15f,0.00f, 13.f,0.68f, 0.055f,0.18f,0.095f},
};
typedef struct {
  float x,z,yaw,hp,flash,anim,phase,state_t,deadT;
  float tx,tz;  /* committed strike point (armory bile puddle) */
...
static void gen_level(unsigned seed){
  ...
  pperk=(levelNo-1)/5; if(pperk>5)pperk=5;
  bossFloor=(levelNo>1 && levelNo%6==0);
  curse=0;
  if(levelNo>1){
    unsigned ch=24+levelNo*3; if(ch>62)ch=62;
    unsigned h=ihash(seed^(unsigned)levelNo*733u);
    if((h%100)<ch) curse=1+(ihash(h^0x9E3779B9u)%4);
  }
  if(pperk && curse && (xs()%100)<(unsigned)(pperk*7)) curse=0;
  ...
  /* rooms: the place grows more labyrinthine as it gets hungry. */
  int maxrooms=7+levelNo/2; if(maxrooms>12)maxrooms=12;
  for(int tries=0; tries<90+levelNo*4 && nrooms<maxrooms; tries++){
  ...
  /* deeper floors eat light; blackout floors eat it with teeth. */
  { float dm=1.0f/(1.0f+levelNo*0.045f); if(curse==1)dm*=0.56f;
    for(int q=0;q<nlights;q++){ lights[q].r*=dm; lights[q].cr*=dm; lights[q].cg*=dm; lights[q].cb*=dm; } }
  ...
  for(int r=1;r<nrooms;r++){
    ...
    for(int k=0;k<n && nitems<MAXITEM;k++){
      if(curse==3 && (xs()%3)==0)continue;
      Item*it=&items[nitems++];
      it->x=(rooms[r].x+1+frand()*(rooms[r].w-2))*CELL;
      it->z=(rooms[r].y+1+frand()*(rooms[r].h-2))*CELL;
      unsigned rr=xs()%(14+levelNo*2); it->type = rr<2?0 : rr<7?1 : rr<10?2 : 3;
      if(levelNo>5 && it->type==3 && (xs()&1)) it->type=2;
      if(it->type==3 && (xs()%5)<3) it->type=6;    /* 60/40 dyn vs rockets — rockets stay the exit-room clearer */
      if(levelNo>3 && (xs()%100)<(unsigned)(2+pperk)) it->type=4+(xs()&1);
      it->taken=0;
    }
  }
  ...
  int depth=levelNo, hard=depth+(curse==2?3:curse==4?4:0); if(hard>20)hard=20;
  for(int r=1;r<nrooms && nen<MAXENEMY-3-(bossFloor?5:0);r++){
    int big = rooms[r].w*rooms[r].h>30;
    int n=1+rooms[r].w*rooms[r].h/16+depth/5; if(n>4)n=4;
    int kind; unsigned roll=xs()%100;
    if(r==nrooms-1){ kind=EK_BRUTE; }                          /* exit guardian */
    else if(roll < (unsigned)(5+hard*3))  kind=EK_BRUTE;
    else if(roll < (unsigned)(18+hard*3)) kind=EK_BRUISER;
    else if(roll < (unsigned)(34+hard*3)) kind=EK_SKITTER;
    else if(roll < (unsigned)(50+hard*2)) kind=EK_ARMORY_SPITTER;
    else if(roll < (unsigned)(68+hard))   kind=EK_SPITTER;
    else                                  kind=EK_CRAWLER;
    if(curse==1 && kind==EK_CRAWLER && (xs()&1))kind=EK_SPITTER;       /* blackout: corridor denial */
    if(curse==2 && roll<70)kind=EK_SKITTER;                            /* swarm: ankle knives */
    if(curse==3 && (kind==EK_SPITTER||kind==EK_ARMORY_SPITTER))kind=EK_BRUISER; /* famine: ammo tax */
    if(kind==EK_SKITTER) n+=2+(curse==2);                       /* swarm */
    ...
    if(curse==1 && (kind==EK_SPITTER||kind==EK_ARMORY_SPITTER))n++;
    if(n>5)n=5;
    for(int k=0;k<n && nen<MAXENEMY-3-(bossFloor?5:0);k++){
      Enemy*e=&en[nen++];
      ...
      e->phase=frand()*6.28f; e->state_t=e->tx=e->tz=e->deadT=0;
    }
  }
  ...
  if(nen<MAXENEMY){
    ...
    e->state=0; e->flash=0; e->anim=0; e->phase=frand()*6.28f; e->state_t=e->tx=e->tz=e->deadT=0;
  }
  /* make every branch creature eligible on every floor, without removing random mix */
  for(int kk=0; kk<EK_COUNT && nrooms>1 && !bossFloor; kk++){
  ...
  if(curse==4 && nen<MAXENEMY && nrooms>2){                    /* hunt: one more tall bastard */
    int r=nrooms/2; Enemy*e=&en[nen++];
    e->x=(rooms[r].x+1+frand()*(rooms[r].w-2))*CELL; e->z=(rooms[r].y+1+frand()*(rooms[r].h-2))*CELL;
    e->yaw=frand()*2*PI; e->kind=EK_BRUTE; e->hp=ekind[EK_BRUTE].hp*0.82f;
    e->state=0; e->flash=0; e->anim=0; e->phase=frand()*6.28f; e->state_t=e->tx=e->tz=e->deadT=0;
  }

  if(bossFloor && nrooms>2){                                     /* breach floor: fat brute + spitters */
    int r=nrooms-1;
    if(nen<MAXENEMY){ Enemy*e=&en[nen++]; e->x=exitx-1.5f; e->z=exitz; e->yaw=frand()*2*PI; e->kind=EK_BRUTE; e->hp=ekind[EK_BRUTE].hp*2.35f; e->state=0; e->flash=0; e->anim=0; e->phase=frand()*6.28f; e->state_t=e->tx=e->tz=e->deadT=0; }
    for(int q=0;q<4 && nen<MAXENEMY;q++){ Enemy*e=&en[nen++]; e->x=(rooms[r].x+1+frand()*(rooms[r].w-2))*CELL; e->z=(rooms[r].y+1+frand()*(rooms[r].h-2))*CELL; e->yaw=frand()*2*PI; e->kind=(q&1)?EK_SPITTER:EK_ARMORY_SPITTER; e->hp=ekind[e->kind].hp*1.25f; e->state=0; e->flash=0; e->anim=0; e->phase=frand()*6.28f; e->state_t=e->tx=e->tz=e->deadT=0; }
  }

  /* floor 8+ sometimes carries an unkillable debt collector.  It is not always
   * present, and even on marked floors it arrives on a random timer. */
  stOn=stMode=0; stFreeze=stHitCD=stHop=stAnim=0; stx=startx; stz=startz; styaw=0; stT=999;
  if(levelNo>7 || bossFloor){
    int ch=17+(levelNo-8)*7-pperk*4; if(ch>58)ch=58; if(ch<12)ch=12;
    if(bossFloor || (int)(xs()%100)<ch){ stOn=1; stT=bossFloor?4.0f+frand()*10.0f:10.0f+frand()*28.0f; }
  }
...
static float pls(float f,double t,float d){ return saw((float)(f*t))<d?1.0f:-1.0f; }
...
static void audio_cb(void*ud,Uint8*stream,int len){
  ...
  for(int i=0;i<n;i++){
    float s=0;
    if(stOn){
    /* BLACK SIGNAL: tiny fake-tracker loop (hook/drop), generated not sampled */
    static const signed char hk[32]={0,3,5,7,10,7,5,3,0,3,5,12,10,7,5,3,0,-1,3,5,7,5,3,-1,0,3,7,10,12,10,7,3};
    static const signed char br[8]={0,0,-1,-1,-5,-5,-2,-2};
    float spm=(bossFloor?132.0f:stMode==2?124.0f:112.0f)*4.0f/60.0f, q=(float)(gt*spm); int k=(int)q,pos=k&15,bar=(k>>4)&31; q-=k;
    int drop=(bar>=16&&bar<20&&!bossFloor), ret=bar>=20||bossFloor||stMode==2, hard=(bar==16&&pos<8&&!bossFloor); float sd=1.0f/spm;
    lpn+=0.01f*(arand()-lpn);
    s+=lpn*0.14f + sinf((float)(gt*2*PI*30.86))*0.012f;
    if(!hard){
      float age=((pos<8?pos:pos-8)+q)*sd, f=61.735f*expf((br[(bar>>1)&7]-12)*0.057762f);
      if(age<(ret?0.18f:0.75f)) s+=pls(f,gt,0.38f)*expf(-age*(ret?12.0f:3.5f))*0.16f;
      if(!drop && !(pos&1)){ f=61.735f*expf((hk[((bar&4)?16:0)+pos]+12+(ret&&(pos==4||pos==12)?12:0))*0.057762f); s+=pls(f,gt,0.30f)*expf(-q*2.7f)*0.085f; }
      if(!drop && ((0x55ad>>pos)&1)){ signed char a[4]={0,3,7,10}; f=61.735f*expf((a[(pos+bar)&3]+12)*0.057762f); s+=pls(f,gt,0.16f)*expf(-q*5.0f)*0.04f; }
      if(drop && (pos==0||pos==8)){ f=61.735f*expf((signed char[]){12,11,7,10}[bar-16]*0.057762f); s+=sinf((float)(gt*2*PI*f))*0.08f*expf(-q*0.5f); }
      if(stMode==2||bossFloor){ f=61.735f*expf((24+((bar+pos)&3)+(bossFloor?7:0))*0.057762f); if(pos==1||pos==6||pos==11||(bossFloor&&pos==4))s+=pls(f,gt,0.09f)*expf(-q*2.3f)*0.065f; }
      if(!drop){ float ka=((pos<8?pos:pos-8)+q)*sd; if(ka<0.20f)s+=sinf((float)(gt*2*PI*(38+80*expf(-ka*35))))*expf(-ka*12)*0.20f;
        if(pos>=12){ float sa=(pos-12+q)*sd; if(sa<0.16f)s+=arand()*expf(-sa*18)*0.08f; }
        if(pos&1)s+=arand()*expf(-q*16)*0.025f;
      }
    }
    } else {
    /* DRAGLINE: dirty industrial horror-stomp, generated not sampled */
    static const signed char rf[16]={0,0,0,3,0,-2,0,5,0,0,7,5,3,0,-2,0};
    static const signed char hk[16]={0,3,5,7,5,3,0,-2,0,3,7,10,7,5,3,0};
    float bpm=124.0f;
    float spm=bpm*4.0f/60.0f, q=(float)(gt*spm); int k=(int)q,pos=k&15,bar=(k>>4)&31,slow=(bar&15)>11; q-=k;
    if(slow) q*=0.55f;
    float sd=1.0f/spm, age=q*sd, f, env;
    lpn+=0.015f*(arand()-lpn);
    s+=lpn*0.10f + sinf((float)(gt*2*PI*36.7f))*0.018f + sinf((float)(gt*2*PI*55.0f))*0.010f;
    if(slow?(pos==0||pos==8):pos==0||pos==4||pos==8||pos==12||((bar&3)==3&&pos==14)){
      f=43.0f+96.0f*expf(-age*25.0f);
      s+=sinf((float)(gt*2*PI*f))*expf(-age*10.5f)*0.32f + arand()*expf(-age*34.0f)*0.06f;
    }
    if((slow?pos==8:pos==4||pos==12)){
      s+=arand()*expf(-age*17.0f)*0.16f + pls(178.0f,gt,0.34f)*expf(-age*22.0f)*0.08f;
    }
    if((pos&3)==2||(((bar&7)==5||(bar&7)==7)&&(pos==3||pos==11||pos==15)))
      s+=arand()*expf(-q*18.0f)*0.028f;
    if((bar&7)==7&&pos>=13)
      s+=(arand()*0.70f+saw((410.0f+35.0f*pos)*(float)gt)*0.30f)*expf(-age*32.0f)*0.075f;

    { int deg=rf[pos]; if((bar&3)==3){ if(pos==10)deg+=5; else if(pos==11)deg+=7; else if(pos==14)deg+=10; }
      f=61.735f*expf((3+deg)*0.057762f);
      env=expf(-age*(pos==3||pos==7||pos==11||pos==15?18.0f:13.0f));
      s+=(pls(f,gt,0.41f)*0.23f+saw((float)(f*0.50f*gt))*0.22f+pls(f*2.0f,gt,0.28f)*0.08f)*env*0.72f;
    }
    if((bar&7)>=4 && !(pos&1)){
      f=61.735f*expf((27+hk[pos]+((bar&8)&&pos>7?12:0))*0.057762f);
      s+=(sinf((float)(gt*2*PI*f))*0.11f+pls(f*2.0f,gt,0.20f)*0.055f+saw((float)(f*0.5f*gt))*0.035f)*expf(-q*(slow?1.4f:2.7f));
    }
    if((bar&3)==1&&(pos==2||pos==7||pos==13)){
      f=61.735f*expf((34+hk[(pos+bar)&15]+((bar&4)?7:0))*0.057762f);
      s+=sinf((float)(gt*2*PI*f))*expf(-q*2.6f)*0.055f;
    }
    if(pos==1||pos==6||pos==10){
      f=61.735f*expf((39+(signed char[]){12,10,7,5}[(pos>>2)&3])*0.057762f);
      s+=(saw((float)(f*gt))*0.09f+pls(f*1.01f,gt,0.18f)*0.05f)*expf(-q*2.3f)*0.75f;
    }
    if(((bar&7)==6||(bar&7)==7)&&pos==10)
      s+=(arand()*0.10f+pls(95.0f,gt,0.28f)*0.055f)*expf(-q*8.0f);
    }
...
static int pweapon=0;        /* 0 pistol, 1 shotgun, 2 rocket, 3 dynamite */
static float prockets,pdyn,pspdT,pdmgT;
static float fireCD,dmgFlash,stepT,shake,kick,flashT,bobT,gtime,floorT;
static unsigned gseed=0xC0FFEEu,runSeed=0xC0FFEEu,levSeed;
static int mapMode=0,bestDepth=0,seedEdit=0,seedLen=8,copied=0,levNo=1,levW=0,startLev=1,tookHit=0,stSeen=0;
static int winPct,winScore,runScore,levScore,winMissE,winMissI,winBon,winEm[EK_COUNT],winIm[7];
static char seedTxt[9]="00C0FFEE";
static float levHp,levAmmo,levShells,levRockets,levDyn,levSpd,levDmg;

static void seed_str(void){ snprintf(seedTxt,9,"%08X",runSeed); seedLen=8; }
static void mark_level(void){ levSeed=gseed; levNo=levelNo; levScore=runScore; levHp=php; levAmmo=pammo; levShells=pshells; levRockets=prockets; levDyn=pdyn; levW=pweapon; levSpd=pspdT; levDmg=pdmgT; copied=0; }
static void clear_runtime(void){
  fireCD=dmgFlash=stepT=shake=kick=flashT=bobT=floorT=0; tookHit=stSeen=0;
...
static void reset_game(void){
  levelNo=1; gseed=runSeed; pspdT=pdmgT=runScore=0;
  for(;levelNo<startLev;levelNo++)gseed=ihash(gseed^(0x9e3779b9u+(unsigned)levelNo*0x85ebca6bu));
  start_level();
  php=100; pammo=32+startLev; pshells=7+startLev/3; prockets=1+startLev/8; pdyn=2+startLev/6; pweapon=0;
  if(pammo>99)pammo=99;
  if(pshells>50)pshells=50;
  if(prockets>20)prockets=20;
  if(pdyn>15)pdyn=15;
  mark_level();
}
static void next_level(void){
  runScore=winScore;
  int up=levelNo/5;
  float hp=php+(levelNo<4?12:7)+up, ammo=pammo+(curse==3?3:5)+up*2, sh=pshells+1+(up>1), rk=prockets+(levelNo%4==0)+(up>2 && levelNo%5==0), dn=pdyn+1+(levelNo%3==0);
  if(hp>100)hp=100;
  if(ammo>99)ammo=99;
  if(sh>50)sh=50;
  if(rk>20)rk=20;
  ...
  levelNo++;
  start_level();
  php=hp; pammo=ammo; pshells=sh; prockets=rk; pdyn=dn;
  mark_level();
}
static void finish_level(void){
  memset(winEm,0,sizeof winEm); memset(winIm,0,sizeof winIm);
  winMissE=winMissI=winBon=0;
  for(int i=0;i<nen;i++) if(en[i].state!=4){ winEm[en[i].kind]++; winMissE++; }
  for(int i=0;i<nitems;i++) if(!items[i].taken){ winIm[items[i].type]++; winMissI++; }
  int total=nen+nitems, done=total-winMissE-winMissI;
  winPct=total?done*100/total:100;
  winScore=levScore+levelNo*100+(nen-winMissE)*50+(nitems-winMissI)*35+winPct*10;
#define AW(bit,pts) do{winBon|=bit;winScore+=pts;}while(0)
  if(!winMissE&&!winMissI) AW(1,1000);
  if(!tookHit) AW(2,750);
  if(floorT<70.0f+levelNo*2.0f) AW(4,500);
  if(curse) AW(8,500);
  if(bossFloor) AW(16,750);
  if(stSeen) AW(32,500);
  if(!winMissE&&!winMissI&&!tookHit) AW(64,1500);
#undef AW
  gstate=ST_WIN; sfx(V_WIN); SDL_SetRelativeMouseMode(SDL_FALSE);
}
static void apply_seed(void){ if(seedLen){ runSeed=gseed=(unsigned)strtoul(seedTxt,0,16); seed_str(); reset_game(); } seedEdit=0; }
static void replay_level(void){ levelNo=levNo; gseed=levSeed; runScore=levScore; pspdT=levSpd; pdmgT=levDmg; start_level(); php=levHp; pammo=levAmmo; pshells=levShells; prockets=levRockets; pdyn=levDyn; pweapon=levW; copied=0; }
...
static void hurt_enemy(int i,float dmg,float hx,float hy,float hz){
  ...
  sfx(V_TICK);
  if(pweapon==1&&e->state!=4){ float dx=e->x-px,dz=e->z-pz,d=sqrtf(dx*dx+dz*dz); if(d>0.01f)move_circ(&e->x,&e->z,dx/d*0.32f,dz/d*0.32f,ekind[e->kind].rad*0.5f); }
...
static void hurt_player(float dmg){
  tookHit=1; php-=dmg; dmgFlash=0.5f; shake=0.3f; sfx(V_HURT);
  if(php<=0 && gstate!=ST_DEAD){
    php=0; winMissE=winMissI=0;
    for(int i=0;i<nen;i++) if(en[i].state!=4)winMissE++;
    for(int i=0;i<nitems;i++) if(!items[i].taken)winMissI++;
    int total=nen+nitems, done=total-winMissE-winMissI;
    winPct=total?done*100/total:100;
    winScore=levScore+levelNo*100+(nen-winMissE)*50+(nitems-winMissI)*35+winPct*10;
    gstate=ST_DEAD; if(levelNo>bestDepth)bestDepth=levelNo; SDL_SetRelativeMouseMode(SDL_FALSE);
  }
}
static void stalker_place(int near){
  for(int q=0;q<42;q++){
    float x,z;
    if(near){ float a=frand()*2*PI, rr=4.8f+frand()*7.8f; x=px+sinf(a)*rr; z=pz-cosf(a)*rr; }
    else { int r=1+(int)(xs()%(nrooms>1?nrooms-1:1)); x=(rooms[r].x+1+frand()*(rooms[r].w-2))*CELL; z=(rooms[r].y+1+frand()*(rooms[r].h-2))*CELL; }
    float dx=x-px,dz=z-pz;
    if(dx*dx+dz*dz>8.0f && circ_free(x,z,0.68f)){
      stx=x; stz=z; styaw=atan2f(px-stx,stz-pz);
      add_templ(stx,1.0f,stz,6.2f,0.18f,2.2f,0.20f,3.2f);
      spawn_parts(18,stx,1.0f,stz,3.8f,0.9f,0.15f,1.5f);
      sfx(V_SPIT); shake=0.18f; return;
    }
  }
  stx=exitx; stz=exitz;
}
static void hurt_stalker(float dmg,float hx,float hy,float hz){
  if(!stOn||stMode!=2)return;
  float f=dmg>30?2.4f:1.15f; if(stFreeze<f)stFreeze=f;
  stHitCD=0.8f;
  add_templ(hx,hy,hz,4.8f,0.16f,2.4f,0.25f,3.4f);
  spawn_parts(10,hx,hy,hz,2.4f,1.0f,0.2f,1.6f);
  sfx(V_TICK);
}
static void update_stalker(float dt){
  if(!stOn)return;
  stAnim+=dt*6.0f;
  if(stHitCD>0)stHitCD-=dt;
  if(stMode==0){
    stT-=dt;
    if(stT<=0){ stSeen=1; stalker_place(1); stMode=1; stT=1.65f; add_templ(px,1.2f,pz,5.0f,0.12f,1.7f,0.15f,2.8f); }
    return;
  }
  if(stMode==1){
    stT-=dt;
    add_templ(stx,1.1f,stz,3.0f,0.06f,1.7f,0.12f,2.4f);
    if(stT<=0){ stMode=2; stT=8.5f+frand()*5.5f; stHop=2.5f+frand()*3.0f; stFreeze=0; stHitCD=0; }
    return;
  }
  if(stMode==2){
    stT-=dt;
    if(stT<=0){
      add_templ(stx,1.0f,stz,6.0f,0.18f,1.8f,0.10f,2.8f);
      spawn_parts(16,stx,1.0f,stz,3.2f,0.8f,0.12f,1.4f);
      stMode=0; stT=18.0f+frand()*38.0f-(levelNo>12?6.0f:0); if(stT<10)stT=10; return;
    }
    if(stFreeze>0){ stFreeze-=dt; add_templ(stx,1.15f,stz,2.7f,0.05f,0.7f,0.9f,3.0f); return; }
    stHop-=dt;
    if(stHop<=0){ stalker_place(xs()&1); stHop=2.7f+frand()*3.6f; }
    float dx=px-stx,dz=pz-stz,d=sqrtf(dx*dx+dz*dz); if(d<0.001f)d=0.001f;
    styaw=atan2f(dx,-dz);
    if(d>1.35f)move_circ(&stx,&stz,dx/d*3.35f*dt,dz/d*3.35f*dt,0.68f);
    else if(stHitCD<=0){ hurt_player(6.0f+(levelNo>10?2.0f:0.0f)); stHitCD=0.82f; shake=0.28f; }
  }
}
static void spawn_proj(int type,int owner,float x,float y,float z,float vx,float vy,float vz){
  for(int i=0;i<MAXPROJ;i++) if(!proj[i].live){
    proj[i]=(Proj){x,y,z,vx,vy,vz, type==PJ_ROCKET?4.0f:type==PJ_DYN?1.8f:3.0f, type,owner,1};
    return; }
}
static void dyn_boom(float x,float y,float z){
  float pm=pdmgT>0?1.65f:1.0f;
  sfx(V_BOOM); shake=0.24f;
  add_templ(x,y,z,4.6f,0.18f,2.2f,1.25f,0.35f);
  spawn_parts(24,x,y,z,4.0f,1.0f,0.45f,0.16f);
  float R=2.25f;
  for(int i=0;i<nen;i++){
    if(en[i].state==4)continue;
    float ex=en[i].x-x, ez=en[i].z-z, ed=sqrtf(ex*ex+ez*ez);
    if(ed<R){ float f=1.0f-ed/R; hurt_enemy(i, (10.0f+50.0f*f)*pm, en[i].x,0.6f,en[i].z); }
  }
  float dx=px-x, dz=pz-z, d=sqrtf(dx*dx+dz*dz);
  if(d<R){ float f=1.0f-d/R; hurt_player(16.0f*f); }
}
static void explode(float x,float y,float z){
  ...
  for(int i=0;i<nen;i++){
    ...
    if(d<R){ float f=1.0f-d/R; hurt_enemy(i, (14.0f+66.0f*f)*pm, en[i].x,0.6f,en[i].z); }
  }
  if(stOn&&stMode==2){ float dx=stx-x,dz=stz-z,d=sqrtf(dx*dx+dz*dz); if(d<R){ float f=1.0f-d/R; hurt_stalker(80.0f*f,stx,1.0f,stz); } }
...
static void fire_ray(float dx,float dy,float dz,float dmg,float maxd){
  ...
  if(stOn&&stMode==2){
    float cx=stx-ox,cy=1.08f-oy,cz=stz-oz,b=cx*dx+cy*dy+cz*dz,r=0.92f;
    if(b>0){ float d2=cx*cx+cy*cy+cz*cz-b*b; if(d2<r*r){ float t=b-sqrtf(r*r-d2); if(t<th){ th=t; hit=-2; } } }
  }
  if(hit==-2) hurt_stalker(dmg, ox+dx*th,oy+dy*th,oz+dz*th);
  else if(hit>=0) hurt_enemy(hit,dmg, ox+dx*th,oy+dy*th,oz+dz*th);
...
static void fire(void){
  ...
  if(pweapon==1){                                       /* shotgun: close-range meat grinder */
    ...
    for(int p=0;p<10;p++){
      ...
      fire_ray(ax*il,ay*il,az*il,8.0f*pm,22.0f);
    }
    return;
  }
  ...
  if(pweapon==3){                                       /* dynamite: lobbed timer charge */
    if(pdyn<1){ sfx(V_CLICK); fireCD=0.25f; return; }
    pdyn--; fireCD=0.95f; flashT=0.05f; kick=0.9f;
    sfx(V_SPIT);
    /* throw from the player's "hand" with a forward arc; PJ_DYN gravity + 1.8s life handles the rest */
    spawn_proj(PJ_DYN,0, px+dx*0.55f,EYE+dy*0.55f-0.08f,pz+dz*0.55f, dx*9.5f, dy*9.5f+3.0f, dz*9.5f);
    add_templ(px+dx*0.6f,EYE+dy*0.6f-0.1f,pz+dz*0.6f,2.2f,0.06f,2.2f,1.5f,0.4f);
    return;
  }
  ...
  fire_ray(dx,dy,dz,12.0f*pm,40.0f);
}
...
static void update_projectiles(float dt){
  for(int i=0;i<MAXPROJ;i++){
    ...
    p->life-=dt;
    if(p->life<=0){ if(p->type==PJ_ROCKET)explode(p->x,p->y,p->z); else if(p->type==PJ_DYN)dyn_boom(p->x,p->y,p->z); p->live=0; continue; }
    if(p->type!=PJ_ROCKET) p->vy-=(p->type==PJ_DYN?7.8f:4.5f)*dt;  /* gobs/dynamite arc, rockets fly flat */
    ...
    if(!circ_free(nx,nz,0.12f) || ny<0.06f || ny>WALLH-0.08f){
      ...
      else if(p->type==PJ_DYN) dyn_boom(p->x,p->y,p->z);
      else spawn_parts(6,p->x,p->y,p->z,2.0f,0.4f,0.9f,0.3f);
      p->live=0; continue;
    }
    p->x=nx; p->y=ny; p->z=nz;
    if(p->owner==0){                                    /* player rocket/dynamite vs creatures */
      for(int e=0;e<nen;e++){
        ...
        if(dx*dx+dy*dy+dz*dz<rr*rr){
          if(p->type==PJ_DYN) dyn_boom(p->x,p->y,p->z);
          else                explode(p->x,p->y,p->z);
          p->live=0; break;
        }
      }
    } else if(p->type==PJ_GOB){                         /* gob vs player */
...
static void update_enemies(float dt){
  for(int i=0;i<nen;i++){
    ...
    switch(e->state){
      ...
      case 1:{ /* engage */
        ...
        if(k==EK_BRUISER){
          ...
          if(e->deadT<=0 && d>4.2f && d<7.5f && los(e->x,e->z,px,pz)){
            e->state=2; e->state_t=0; e->tx=px; e->tz=pz; e->deadT=-1; break;
          }
        }
        if(k==EK_SPITTER){
          ...
          e->anim+=dt*6.5f;
          if(d<ar && los(e->x,e->z,px,pz)){
            e->state=2; e->state_t=0;
            e->tx=px; e->tz=pz;            /* COMMIT the puddle to where you stand now */
            add_templ(e->tx,0.10f,e->tz,2.2f,0.18f,0.10f,1.6f,0.45f);  /* "it lands here" flash */
          }
        } else {
          if(k==EK_BRUTE){
            e->tx-=dt;
            if(levelNo>1 && e->tx<=0 && d>4.2f && d<16.f && los(e->x,e->z,px,pz)){
              for(int q=0;q<10;q++){ float a=frand()*2*PI, rr=3.0f+frand()*4.0f;
                float nx=px+sinf(a)*rr, nz=pz-cosf(a)*rr;
                if(circ_free(nx,nz,0.55f)&&los(nx,nz,px,pz)){
                  add_templ(e->x,0.9f,e->z,5.0f,0.16f,1.7f,0.25f,2.2f);
                  spawn_parts(12,e->x,0.8f,e->z,3.0f,0.8f,0.2f,1.2f);
                  e->x=nx; e->z=nz; e->tx=(curse==4?3.0f:5.0f)+frand()*1.8f-levelNo*0.08f; if(e->tx<1.8f)e->tx=1.8f;
                  add_templ(nx,0.9f,nz,5.0f,0.18f,2.0f,0.4f,2.6f); sfx(V_SPIT); shake=0.22f; break;
                }}
            }
          }
      ...
      case 2:{ /* windup: melee lunge or armory bile-light attack */
        ...
        float wind=k==EK_ARMORY_SPITTER?0.68f : k==EK_SKITTER?0.20f : k==EK_BRUTE?0.55f : k==EK_BRUISER?(e->deadT<0?1.10f:0.42f) : 0.35f;
        if(k==EK_ARMORY_SPITTER){
          /* danger pool sits at the COMMITTED spot and swells to its true kill size,
           * so the lit ground is the hitbox — step off it and you're clear. */
          float t=e->state_t/wind; if(t>1)t=1;
          float rad=0.8f+1.6f*t;                       /* grows toward the kill radius */
          add_templ(e->tx,0.06f,e->tz,rad,0.06f,0.10f,1.9f,0.45f);           /* wide warning wash */
          add_templ(e->tx,0.10f,e->tz,0.5f+1.0f*t,0.06f,0.20f,2.4f,0.55f);   /* bright core      */
          if(((int)(e->state_t*22))&1)                                        /* sparks off the pool */
            spawn_parts(1,e->tx+(frand()-0.5f)*rad,0.10f,e->tz+(frand()-0.5f)*rad,1.4f,0.30f,1.0f,0.28f);
        }
        if(e->state_t>wind){
          if(k==EK_ARMORY_SPITTER){
            float bx=px-e->tx, bz=pz-e->tz, Rb=1.7f;
            if(bx*bx+bz*bz < Rb*Rb) hurt_player(16);     /* only if you're still in the pool */
            add_templ(e->tx,0.12f,e->tz,5.2f,0.20f,0.32f,2.2f,0.55f);
            spawn_parts(20,e->tx,0.14f,e->tz,3.4f,0.40f,1.0f,0.28f);
            shake=0.25f;
          }else if(k==EK_BRUISER && e->deadT<0){
            float cy=cosf(e->yaw), syw=sinf(e->yaw);
            float sx=e->x+cy*0.62f+syw*0.15f, sy=0.72f, sz=e->z+syw*0.62f-cy*0.15f;
            float tx=e->tx-sx, tz=e->tz-sz, l=sqrtf(tx*tx+tz*tz); if(l<1)l=1;
            float ft=0.78f+l*0.045f;
            spawn_proj(PJ_DYN,1,sx,sy,sz,tx/ft,4.2f,tz/ft);
            add_templ(sx,sy,sz,3.2f,0.15f,1.7f,0.85f,0.32f); sfx(V_SPIT); e->deadT=3.2f;
          }else{
    ...
    /* active enemies visually face the player using their front/eye side. */
    if(e->state!=0 && e->state!=4) e->yaw=atan2f(px-e->x, -(pz-e->z));
...
static void draw_enemy(Enemy*e){
  int k=e->kind;
  if(e->state!=0&&e->state!=4){ float fx=px-e->x,fz=pz-e->z; e->yaw=atan2f(fx,-fz); }
  float vyaw=(e->state!=0&&e->state!=4)?-e->yaw:e->yaw;
  float M[9],R[9],P[9],Z[9],T[9],L[9];
  float maxhp=ekind[k].hp; if(k==EK_BRUTE&&e->hp>maxhp)maxhp=e->hp;
  ...
  float winp = e->state==2 ? e->state_t/(k==EK_SKITTER?0.20f:k==EK_BRUTE?0.55f:k==EK_BRUISER?(e->deadT<0?1.10f:0.42f):0.35f) : 0.0f;

  /* ---- CRAWLER: the original angry cable-insect ---- */
  if(k==EK_CRAWLER){
    float rearup = e->state==2 ? -0.75f*sinf(winp*PI) : 0.10f*sinf(e->anim*0.35f+e->phase);
    m3rotY(R,vyaw); m3rotX(P,rearup); m3mul(M,R,P);
  ...
  if(k==EK_BRUISER){
    ...
    int throwing = (e->state==2 && e->deadT<0);
    for(int side=-1;side<=1;side+=2){
      float swing=sinf(e->anim+(side>0?PI:0))*0.45f; if(e->state==4)swing=1.1f;
      /* during the throw windup, the right arm rears back then snaps forward toward the player */
      if(throwing && side>0){ float t=winp; if(t>1)t=1; swing=-1.10f+2.20f*t; }
    ...
    /* dynamite stick visible in the throwing hand during the windup */
    if(throwing){
      float t=winp; if(t>1)t=1;
      float reach=0.55f+0.35f*t;                 /* arm extends forward as the throw commits */
      float lift=0.20f - 0.10f*t;                /* drops slightly as it snaps down */
      float ho[3]; m3v(M,0.42f,0.55f+lift,-reach,ho);
      glUniform3f(uTint,0.32f,0.10f,0.08f);      /* red dyn stick */
      set_uM(M,e->x+ho[0],0.55f*squash+ho[1]*squash,e->z+ho[2]); box_sh(0.10f,0.10f,0.22f);
      glUniform1f(uEmis,1); glUniform3f(uTint,2.2f,0.85f,0.18f);   /* hot fuse spark */
      set_uM(M,e->x+ho[0],0.66f*squash+ho[1]*squash,e->z+ho[2]); box_sh(0.04f,0.06f,0.04f);
      glUniform1f(uEmis,0);
    }
    return;
  }
  ...
  if(k==EK_SPITTER){
    ...
    m3rotY(R,vyaw); m3rotX(P,lean); m3mul(M,R,P);
    glUniform1f(uBump,0);
  ...
  if(k==EK_SKITTER){
    ...
    m3rotY(R,vyaw); m3rotX(P,-winp*0.5f+scoot); m3mul(M,R,P);
    glUniform1f(uBump,0); glUniform1f(uEmis,0);
  ...
  /* ---- BRUTE: heavy tank, armoured back with cracks that flare as it dies ---- */
  {
    ...
    float bg=e->hp>ekind[EK_BRUTE].hp*1.5f?1.32f:1.0f;
    float A[9]; memcpy(A,M,36); m3scl(A,bg,squash*bg,bg);
    set_uM(A,e->x,0.55f*squash*bg+0.04f,e->z); box_sh(1.15f,0.62f,1.45f);
    /* armoured hump */
...
static void draw_stalker(void){
  if(!stOn||stMode==0)return;
  Enemy e; memset(&e,0,sizeof e);
  e.x=stx; e.z=stz; e.yaw=styaw; e.kind=EK_BRUTE; e.hp=ekind[EK_BRUTE].hp; e.state=stFreeze>0?2:1; e.anim=stAnim; e.phase=2.2f; e.flash=stFreeze>0?0.12f:0;
  draw_enemy(&e);
}
...
static void draw_items(void){
  ...
  for(int i=0;i<nitems;i++){
    ...
    if(items[i].type==0) glUniform3f(uTint,2.4f,0.12f,0.12f);       /* health: hi-vis red */
    else if(items[i].type==1) glUniform3f(uTint,2.6f,2.5f,0.10f);   /* bullets: hi-vis yellow */
    else if(items[i].type==2) glUniform3f(uTint,2.6f,0.15f,2.2f);   /* shells: hi-vis magenta */
    else if(items[i].type==4) glUniform3f(uTint,0.12f,2.7f,0.18f);  /* speed: neon green */
    else if(items[i].type==5) glUniform3f(uTint,2.8f,0.10f,1.10f);  /* rage: hot pink */
    else if(items[i].type==6) glUniform3f(uTint,2.7f,0.55f,0.10f);   /* dynamite: hot orange */
    else                 glUniform3f(uTint,0.10f,2.4f,2.8f);        /* rockets: electric cyan */
...
static void draw_world(float camx,float camy,float camz){
  ...
  draw_stalker();
  draw_items();
  glUniform1f(uBump,0); glUniform1f(uEmis,1);
  for(int i=0;i<MAXPROJ;i++){ Proj*p=&proj[i]; if(!p->live||(p->type!=PJ_ROCKET&&p->type!=PJ_DYN))continue;
    m3rotY(I,atan2f(p->vx,-p->vz));
    if(p->type==PJ_DYN){
      glUniform3f(uTint,0.85f,0.12f,0.06f);
      set_uM(I,p->x,p->y,p->z); box_sh(0.08f,0.08f,0.34f);
      float ro[3]; m3v(I,0,0,-0.22f,ro); glUniform3f(uTint,3.0f,1.25f,0.18f);
      set_uM(I,p->x+ro[0],p->y+ro[1],p->z+ro[2]); box_sh(0.055f,0.055f,0.055f);
    }else{
      glUniform3f(uTint,2.4f,0.75f,0.12f);
      set_uM(I,p->x,p->y,p->z); box_sh(0.13f,0.13f,0.48f);
      float ro[3]; m3v(I,0,0,-0.30f,ro); glUniform3f(uTint,3.0f,1.6f,0.22f);
      set_uM(I,p->x+ro[0],p->y+ro[1],p->z+ro[2]); box_sh(0.11f,0.11f,0.08f);
    }
  }
  glUseProgram(0);
  ...
  for(int i=0;i<MAXPROJ;i++){ Proj*p=&proj[i]; if(!p->live)continue;
    ...
    } else if(p->type==PJ_DYN)
      billboard(p->x,p->y,p->z,0.18f,1.0f,0.45f,0.12f,0.9f,pyaw+90);
    else
...
static void draw_gun(void){
  ...
  float gx=0.21f, gy=-0.22f, gz=-0.45f;
  if(pweapon==1){ gx=0.225f; gy=-0.238f; gz=-0.47f; }
  else if(pweapon==3){ gx=0.235f; gy=-0.205f; gz=-0.40f; }
  glTranslatef(gx, gy+bob, gz+kz);
  glRotatef(-kick*(pweapon==2?9:(pweapon==1?11:pweapon==3?14:6)),1,0,0);
  ...
  static const struct PartB shotgun[6]={
    {-0.052f,0.042f,-0.215f, 0.072f,0.072f,0.48f},
    { 0.052f,0.042f,-0.215f, 0.072f,0.072f,0.48f},
    {-0.052f,0.042f,-0.485f,0.088f,0.088f,0.055f},
    { 0.052f,0.042f,-0.485f,0.088f,0.088f,0.055f},
    {0,-0.030f,0.040f,      0.195f,0.145f,0.230f},
    {0,-0.145f,0.105f,      0.068f,0.132f,0.085f}};
  ...
  /* dynamite held: forearm + stick body + end caps + fuse (no hand — keeps the silhouette stick-like, not gun-like) */
  static const struct PartB dynamite[5]={
    {0,-0.090f, 0.040f, 0.082f,0.078f,0.300f},  /* forearm — extends forward to meet the stick */
    {0, 0.005f,-0.18f,  0.055f,0.055f,0.22f},   /* stick body */
    {0, 0.005f,-0.30f,  0.060f,0.060f,0.018f},  /* far cap    */
    {0, 0.005f,-0.06f,  0.060f,0.060f,0.018f},  /* near cap   */
    {0, 0.050f,-0.30f,  0.012f,0.060f,0.012f}}; /* fuse       */
  const struct PartB*parts_ = pweapon==3?dynamite:(pweapon==2?launcher:(pweapon==1?shotgun:pistol));
  int np = pweapon==3?5:(pweapon==2?4:(pweapon==1?6:3));
  /* if a player-thrown stick is still in flight, the hand is empty — show only the arm */
  if(pweapon==3){
    for(int q=0;q<MAXPROJ;q++) if(proj[q].live && proj[q].type==PJ_DYN && proj[q].owner==0){ np=1; break; }
  }
  float muzz = pweapon==2?-0.52f:(pweapon==1?-0.53f:(pweapon==3?-0.32f:-0.34f));
  for(int p=0;p<np;p++){
    ...
    float rg=1.04f, rb=1.10f, mt=1.0f;
    if(pweapon==2){ rg=1.18f; rb=0.95f; }
    else if(pweapon==3){
      if(p==0)      { rg=0.82f; rb=0.68f; }            /* forearm: shotgun-stock warm wood/leather */
      else if(p==4) { rg=0.55f; rb=0.20f; mt=3.2f; }   /* fuse glows hot */
      else          { rg=0.32f; rb=0.28f; mt=1.4f; }   /* red stick body + caps */
    }
    else if(pweapon==1){
      if(p<=1){ rg=1.05f; rb=0.95f; }
      else if(p<=3){ rg=0.75f; rb=0.90f; mt=0.28f; }
      else { rg=0.82f; rb=0.68f; }
    }
    glBegin(GL_QUADS);
    for(int f=0;f<6;f++){
      float k2=shade[f]*0.22f*mt;
  ...
  if(flashT>0){
    ...
    glBegin(GL_QUADS);
    if(pweapon==1){
      for(int q=-1;q<=1;q+=2){ float ox=q*0.075f;
...
static void rect2(float x,float y,float w,float h){
  glVertex2f(x,y);glVertex2f(x+w,y);glVertex2f(x+w,y+h);glVertex2f(x,y+h);
}
static void draw_map(void){
  float s=4.55f, ox=30, oy=58;
  glDisable(GL_TEXTURE_2D);
  glColor4f(0,0,0,0.72f);
  glBegin(GL_QUADS); rect2(ox-5,oy-5,G*s+10,G*s+10); glEnd();
  glBegin(GL_QUADS);
  for(int z=0;z<G;z++)for(int x=0;x<G;x++) if(!grid[z][x]){
    float xx=ox+x*s, yy=oy+z*s;
    glColor4f(0.09f,0.13f,0.12f,0.82f); rect2(xx,yy,s*0.82f,s*0.82f);
  }
  if(curse!=1&&nrooms>0){
    Room*er=&rooms[nrooms-1];
    float exx=ox+er->x*s-1, eyy=oy+er->y*s-1;
    float ew=(er->w-1)*s+s*0.82f+2, eh=(er->h-1)*s+s*0.82f+2;
    glColor4f(0.08f,0.42f,0.12f,0.42f); rect2(exx,eyy,ew,eh);
    glColor4f(0.22f,1.0f,0.35f,0.95f);
    rect2(exx,eyy,ew,1.4f); rect2(exx,eyy+eh-1.4f,ew,1.4f);
    rect2(exx,eyy,1.4f,eh); rect2(exx+ew-1.4f,eyy,1.4f,eh);
  }
  if(curse!=1){ glColor4f(0.1f,1.0f,0.45f,0.95f); rect2(ox+exitx/CELL*s-3.5f,oy+exitz/CELL*s-3.5f,7,7); }
  for(int i=0;i<nitems;i++)if(!items[i].taken){
    float xx=ox+items[i].x/CELL*s, yy=oy+items[i].z/CELL*s;
    if(items[i].type==0)glColor4f(0.9f,0.1f,0.1f,0.8f); else if(items[i].type==1)glColor4f(0.9f,0.8f,0.25f,0.8f); else if(items[i].type>3)glColor4f(1.0f,0.25f,1.0f,0.9f); else glColor4f(0.35f,0.65f,1,0.8f);
    rect2(xx-1.5f,yy-1.5f,3,3);
  }
  for(int i=0;i<nen;i++)if(en[i].state!=4){
    float xx=ox+en[i].x/CELL*s, yy=oy+en[i].z/CELL*s, a=(en[i].kind==EK_BRUTE?1.0f:0.72f);
    if(curse==1 && en[i].kind!=EK_BRUTE)continue;                 /* blackout hides small blips */
    glColor4f(en[i].kind==EK_SPITTER||en[i].kind==EK_ARMORY_SPITTER?0.05f:1.0f,en[i].kind==EK_SKITTER?0.55f:0.85f,en[i].kind==EK_BRUTE?1.0f:0.12f,a);
    rect2(xx-2,yy-2,4,4);
  }
  if(stOn&&stMode){ float xx=ox+stx/CELL*s, yy=oy+stz/CELL*s; glColor4f(0.85f,0.12f,1.0f,0.95f); rect2(xx-3.5f,yy-3.5f,7,7); }
  glColor4f(1,1,1,1); rect2(ox+px/CELL*s-2.5f,oy+pz/CELL*s-2.5f,5,5);
  glEnd();
  glColor4f(0.55f,0.95f,0.65f,0.8f); draw_text(ox,oy+G*s+8,1.55f,"M MAP");
}
static void draw_hud(void){
  ...
  if(gstate==ST_PLAY){
    ...
    int curammo = pweapon==0 ? (int)pammo : (pweapon==1 ? (int)pshells : (pweapon==2 ? (int)prockets : (int)pdyn));
    char am[8]; snprintf(am,8,"%02d",curammo);
    glColor4f(1,0.85f,0.3f,0.95f);
    draw_text(WINW-30-textw(am,5),WINH-66,5,am);
    const char*alabel = pweapon==0 ? "AMMO" : (pweapon==1 ? "SHELLS" : (pweapon==2 ? "ROCKETS" : "DYN"));
    glColor4f(1,1,1,0.7f); draw_text(WINW-30-textw(alabel,2.2f),WINH-90,2.2f,alabel);
    { const char*w0="1 PISTOL", *w1="2 SHOTGUN", *w2="3 ROCKET", *w3="4 DYNAMITE";
      ...
      if(pweapon==3)glColor4f(1,0.85f,0.3f,0.95f); else glColor4f(0.45f,0.45f,0.5f,0.8f);
      draw_text(WINW-30-textw(w3,2.0f),WINH-186,2.0f,w3); }
    static const char*bname[NBIOME]={"WORKS","HIVE","VESSEL"};
    static const char*cname[5]={""," BLACKOUT"," SWARM"," FAMINE"," HUNT"};
    char fl[40];
    if(mixTiles) snprintf(fl,40,"FLOOR-%02d MIXED%s%s",levelNo,cname[curse],bossFloor?" BOSS":"");
    else snprintf(fl,40,"FLOOR-%02d %s%s%s",levelNo,bname[curBiome],cname[curse],bossFloor?" BOSS":"");
    glColor4f(0.55f,0.95f,0.65f,0.75f); draw_text(30,26,2.0f,fl);
    float sx=mapMode?255.0f:30.0f;
    if(stOn){ glColor4f(0.9f,0.2f,1.0f,stMode?0.95f:0.45f); draw_text(sx,74,1.8f,stMode==2?"STALKER":"STALKED"); }
    if(pperk){ char pk[16]; snprintf(pk,16,"PERK-%d",pperk); glColor4f(0.55f,0.95f,0.95f,0.65f); draw_text(sx,98,1.6f,pk); }
    if(pspdT>0||pdmgT>0){ char pw[20]; snprintf(pw,20,"%s%s",pspdT>0?"SPEED":"",pdmgT>0?" RAGE":""); glColor4f(1,0.35f,1,0.85f); draw_text(sx,122,1.45f,pw); }
    if(mapMode)draw_map();
    else { glColor4f(0.35f,0.55f,0.45f,0.55f); draw_text(30,50,1.6f,"M MAP"); }
    if(dmgFlash>0){
    ...
    if(gstate==ST_TITLE){
      ...
      draw_text((WINW-textw("NO ASSETS - JUST SEED BAD AIR AND CURSES",2.2f))/2,345,2.2f,"NO ASSETS - JUST SEED BAD AIR AND CURSES");
      char ss[40]; snprintf(ss,40,seedEdit==1?"SEED %s_":"SEED %s - E",seedTxt);
      glColor4f(0.65f,0.9f,0.9f,0.9f); draw_text((WINW-textw(ss,2.3f))/2,405,2.3f,ss);
      snprintf(ss,40,seedEdit==2?"FLOOR %d_":"FLOOR %d - F",startLev);
      draw_text((WINW-textw(ss,2.0f))/2,442,2.0f,ss);
      glColor4f(0.9f,0.8f,0.4f,0.7f+0.3f*sinf(gtime*4));
      draw_text((WINW-textw("CLICK TO GO BELOW",3.4f))/2,485,3.4f,"CLICK TO GO BELOW");
      glColor4f(0.45f,0.45f,0.5f,1);
      draw_text((WINW-textw("WASD MOVE - MOUSE LOOK - LMB FIRE - M MAP - GREEN EXIT",2))/2,560,2,
        "WASD MOVE - MOUSE LOOK - LMB FIRE - M MAP - GREEN EXIT");
    } else if(gstate==ST_DEAD){
      glColor4f(0.85f,0.1f,0.08f,1);
      draw_text((WINW-textw("YOU DIED",10))/2,260,10,"YOU DIED");
      glColor4f(0.8f,0.8f,0.8f,0.8f);
      char ds[64]; snprintf(ds,64,"SCORE %05d",winScore);
      draw_text((WINW-textw(ds,2.7f))/2,350,2.7f,ds);
      snprintf(ds,64,"DEPTH %02d RUN %08X",levelNo,runSeed);
      draw_text((WINW-textw(ds,2.4f))/2,400,2.4f,ds);
      snprintf(ds,64,"FLOOR %02d BEST %02d",levelNo,bestDepth);
      draw_text((WINW-textw(ds,2.4f))/2,445,2.4f,ds);
      draw_text((WINW-textw(copied?"COPIED RUN+FLOOR":"C COPY RUN+FLOOR",1.85f))/2,490,1.85f,
        copied?"COPIED RUN+FLOOR":"C COPY RUN+FLOOR");
      draw_text((WINW-textw("R RETRY - ENTER NEW RUN",1.85f))/2,525,1.85f,"R RETRY - ENTER NEW RUN");
    } else {
      glColor4f(0.3f,1.0f,0.6f,1);
      draw_text((WINW-textw("EXIT REACHED",7))/2,190,7,"EXIT REACHED");
      char b2[128];
      glColor4f(winPct>=100?0.35f:0.8f, winPct>=100?1.0f:0.8f, winPct>=100?0.35f:0.8f,0.9f);
      snprintf(b2,128,"CLEAR %03d%%",winPct); draw_text((WINW-textw(b2,2.4f))/2,290,2.4f,b2);
      snprintf(b2,128,"SCORE %05d",winScore); draw_text((WINW-textw(b2,2.1f))/2,326,2.1f,b2);
      glColor4f(0.95f,0.82f,0.35f,0.9f); draw_text((WINW-textw("BONUS",1.45f))/2,360,1.45f,"BONUS");
      static const char*bn[7]={"FULL CLEAR","NO DAMAGE","FAST EXIT","CURSE FLOOR","BOSS BREACH","STALKER LIVED","CLEAN SWEEP"};
      static const int bv[7]={1000,750,500,500,750,500,1500}; int y=382;
      for(int b=0;b<7;b++) if(winBon&(1<<b)){ snprintf(b2,128,"%s +%d",bn[b],bv[b]); draw_text((WINW-textw(b2,1.05f))/2,y,1.05f,b2); y+=15; }
      glColor4f(0.8f,0.8f,0.8f,0.85f);
      snprintf(b2,128,"MISSED %02d ENEMY %02d PICKUP",winMissE,winMissI); draw_text((WINW-textw(b2,1.18f))/2,y+2,1.18f,b2); y+=18;
      if(winMissE){ static const char*n[EK_COUNT]={"CRAWL","SPIT","SKIT","BRUTE","BRUISR","ACID"};
        int c=0; draw_text(430,y,1.10f,"ENEMY");
        for(int k=0;k<EK_COUNT;k++) if(winEm[k]){ snprintf(b2,128,"%s %d",n[k],winEm[k]); draw_text(520+(c&1)*125,y+(c>>1)*16,1.10f,b2); c++; }
        y+=16+((c+1)>>1)*16;
      }
      if(winMissI){ static const char*n[7]={"HP","AMMO","SHELL","ROCKET","SPEED","RAGE","DYN"};
        int c=0; draw_text(430,y,1.10f,"PICKUP");
        for(int k=0;k<7;k++) if(winIm[k]){ snprintf(b2,128,"%s %d",n[k],winIm[k]); draw_text(520+(c&1)*125,y+(c>>1)*16,1.10f,b2); c++; }
      }
      draw_text((WINW-textw("LMB TO DESCEND",1.7f))/2,575,1.7f,"LMB TO DESCEND");
    }
  }
  glDisable(GL_BLEND);
...
int main(int argc,char**argv){
  ...
  if(!seedSet)gseed=ihash((unsigned)SDL_GetTicks()^(unsigned)(unsigned long)&argc^(unsigned)(unsigned long)argv^0xA53C9E1Du);
  runSeed=gseed; seed_str();
  ...
  while(running){
    ...
    while(SDL_PollEvent(&ev)){
      if(ev.type==SDL_QUIT)running=0;
        ...
        if(d&&gstate==ST_TITLE){
          ...
          if(seedEdit==1){
            if(k==SDLK_RETURN) apply_seed();
            else if(k==SDLK_BACKSPACE&&seedLen) seedTxt[--seedLen]=0;
            else if(h>=0&&seedLen<8){ seedTxt[seedLen++]="0123456789ABCDEF"[h]; seedTxt[seedLen]=0; }
            continue;
          }
          if(seedEdit==2){
            if(k==SDLK_RETURN){ if(startLev<1)startLev=1; reset_game(); seedEdit=0; }
            else if(k==SDLK_BACKSPACE) startLev/=10;
            else if(h>=0&&h<10&&startLev<100) startLev=startLev*10+h;
            continue;
          }
          if(k==SDLK_e){ seedEdit=1; seedLen=0; seedTxt[0]=0; continue; }
          if(k=='f'){ seedEdit=2; startLev=0; continue; }
        }
        if(d&&gstate==ST_DEAD){
          if(k==SDLK_c){ char cb[64]; snprintf(cb,64,"./nervk --seed 0x%08X --floor %d",runSeed,levelNo); SDL_SetClipboardText(cb); copied=1; continue; }
          if(k==SDLK_r){ replay_level(); gstate=ST_PLAY; SDL_SetRelativeMouseMode(SDL_TRUE); continue; }
          if(k==SDLK_RETURN){ reset_game(); gstate=ST_PLAY; SDL_SetRelativeMouseMode(SDL_TRUE); continue; }
        }
        switch(k){
        ...
        if(gstate==ST_TITLE){ if(seedEdit) continue; gstate=ST_PLAY; SDL_SetRelativeMouseMode(SDL_TRUE); }
        else if(gstate==ST_DEAD){ }
      ...
      else if(ev.type==SDL_MOUSEWHEEL && gstate==ST_PLAY){ pweapon=(pweapon+(ev.wheel.y>0?1:3))%4; }
    }
    ...
    if(gstate==ST_PLAY){
      ...
      if(ml>0.01f){
        mx/=ml;mz/=ml;
        float ps=5.2f*(pspdT>0?1.28f:1.0f);
        move_circ(&px,&pz,mx*ps*dt,mz*ps*dt,0.34f);
        bobT+=dt; stepT-=dt;
      ...
      for(int i=0;i<nitems;i++){
        ...
        if(dx*dx+dz*dz<0.8f*0.8f){
          ...
          if(items[i].type==0){ php+=curse==3?18:25; if(php>100)php=100; }
          else if(items[i].type==1){ pammo+=curse==3?8:12; if(pammo>99)pammo=99; }
          else if(items[i].type==2){ pshells+=curse==3?3:5; if(pshells>50)pshells=50; }
          else if(items[i].type==4){ pspdT=12.0f+frand()*5.0f; add_templ(px,1,pz,5,0.2f,0.2f,2.5f,1.0f); }
          else if(items[i].type==5){ pdmgT=10.0f+frand()*4.0f; add_templ(px,1,pz,5,0.2f,2.5f,0.1f,2.4f); }
          else if(items[i].type==6){ pdyn+=2+(curse!=3); if(pdyn>15)pdyn=15; }
          else { prockets+=1+(curse!=3); if(prockets>20)prockets=20; }
        }
      }
      { float dx=exitx-px,dz=exitz-pz;
        if(dx*dx+dz*dz<0.9f*0.9f) finish_level(); }
    }
