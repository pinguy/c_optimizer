...
static int pweapon=0;        /* 0 pistol, 1 shotgun, 2 rocket */
static float prockets,pspdT,pdmgT;
...
static int winPct,winScore,runScore,levScore,winMissE,winMissI,winBon,winEm[EK_COUNT],winIm[6];
static char seedTxt[9]="00C0FFEE";
static float levHp,levAmmo,levShells,levRockets,levSpd,levDmg;

static void seed_str(void){ snprintf(seedTxt,9,"%08X",runSeed); seedLen=8; }
static void mark_level(void){ levSeed=gseed; levNo=levelNo; levScore=runScore; levHp=php; levAmmo=pammo; levShells=pshells; levRockets=prockets; levW=pweapon; levSpd=pspdT; levDmg=pdmgT; copied=0; }
...
static void reset_game(void){
  ...
  start_level();
  php=100; pammo=32+startLev; pshells=7+startLev/3; prockets=1+startLev/8; pweapon=0;
...
static void next_level(void){
  runScore=winScore;
  int up=levelNo/5;
  float hp=php+(levelNo<4?12:7)+up, ammo=pammo+(curse==3?3:5)+up*2, sh=pshells+1+(up>1), rk=prockets+(levelNo%4==0)+(up>2 && levelNo%5==0);
  if(hp>100)hp=100;
  if(ammo>99)ammo=99;
  if(sh>50)sh=50;
  if(rk>20)rk=20;
...
static void replay_level(void){ levelNo=levNo; gseed=levSeed; runScore=levScore; pspdT=levSpd; pdmgT=levDmg; start_level(); php=levHp; pammo=levAmmo; pshells=levShells; prockets=levRockets; pweapon=levW; copied=0; }
...
static void explode(float x,float y,float z){
  ...
  for(int i=0;i<nen;i++){
    ...
    if(d<R){ float f=1.0f-d/R; hurt_enemy(i, (12.0f+58.0f*f)*pm, en[i].x,0.6f,en[i].z); }
  }
  ...
  for(int i=0;i<nen;i++){
    ...
    switch(e->state){
      ...
      case 1:{ /* engage */
        ...
        if(k==EK_BRUISER && d>3.0f && los(e->x,e->z,px,pz))spd*=1.55f;
        if(k==EK_SPITTER){
      ...
      case 2:{ /* windup: melee lunge or armory bile-light attack */
        ...
        float wind=k==EK_ARMORY_SPITTER?0.68f : k==EK_SKITTER?0.20f : k==EK_BRUTE?0.55f : k==EK_BRUISER?0.42f : 0.35f;
...
static void draw_world(float camx,float camy,float camz){
  ...
  for(int i=0;i<MAXPROJ;i++){ Proj*p=&proj[i]; if(!p->live||p->type!=PJ_ROCKET)continue;
    m3rotY(I,atan2f(p->vx,-p->vz)); glUniform3f(uTint,2.4f,0.75f,0.12f);
...
static void draw_gun(void){
  ...
  int np = pweapon==2?4:(pweapon==1?6:3);
  float muzz = pweapon==2?-0.52f:(pweapon==1?-0.53f:-0.34f);
  for(int p=0;p<np;p++){
...
static void draw_hud(void){
  ...
  if(gstate==ST_PLAY){
    ...
    if(gstate==ST_TITLE){
      ...
      if(winMissI){ static const char*n[6]={"HP","AMMO","SHELL","ROCKET","SPEED","RAGE"};
        int c=0; draw_text(430,y,1.10f,"PICKUP");
        for(int k=0;k<6;k++) if(winIm[k]){ snprintf(b2,128,"%s %d",n[k],winIm[k]); draw_text(520+(c&1)*125,y+(c>>1)*16,1.10f,b2); c++; }
      }
