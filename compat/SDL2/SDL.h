...
typedef struct { Uint32 type; Uint32 timestamp, windowID, which; int x, y; } SDL_MouseWheelEvent;
...
typedef union SDL_Event { Uint32 type; SDL_KeyboardEvent key; SDL_WindowEvent window; SDL_MouseMotionEvent motion; SDL_MouseButtonEvent button; SDL_MouseWheelEvent wheel; Uint8 padding[56]; } SDL_Event;
...
SDL_Window *SDL_CreateWindow(const char *title, int x, int y, int w, int h, Uint32 flags); void SDL_DestroyWindow(SDL_Window *window); int SDL_SetWindowFullscreen(SDL_Window *window, Uint32 flags);
int SDL_PollEvent(SDL_Event *event); int SDL_SetRelativeMouseMode(int enabled); int SDL_SetClipboardText(const char *text);
